...
class DOMIT_XPath {
	...
	*/
	function initSearch(&$currArPathSegments) {
		...
		$temp =& $this->callingNode;
		
		if (is_null($currArPathSegments[0])) {
			if (count($currArPathSegments) == 1) {
				//variable path
				$this->searchType = DOMIT_XPATH_SEARCH_VARIABLE;
				$this->globalNodeContainer[] =& $temp->ownerDocument;
			}
			else {
				//absolute path
			...
			if ($temp->uid != $temp->ownerDocument->uid) {
				$this->globalNodeContainer[] =& $temp;
			}
			else {
				$this->globalNodeContainer[] =& $temp;
			}
		}
		//print_r($this->globalNodeContainer);	

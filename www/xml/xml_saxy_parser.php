...
class SAXY_Parser extends SAXY_Parser_Base {
	...
	*/
	function parse ($xmlText) {
		...
		$xmlText = eregi_replace('(<(img|input|form)[^>]*[^/])>','\\1 />',$xmlText);
		$xmlText = eregi_replace('(<(br|center)[^/>]*)>','\\1 />',$xmlText);
		$xmlText = eregi_replace('(</form>|</center>)','',$xmlText);
	...
	*/
	function _fireEndElementEvent($tagName) {
		...
		if ($lastTagName != $tagName) {
			echo "Error: Expecting: $lastTagName found: $tagName <br />";
